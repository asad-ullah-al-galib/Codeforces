...
void answer()
{
    ...
    int kom=*min_element(v.begin(),v.end());
    int besi=*max_element(v.begin(),v.end());
    int komm=count(v.begin(),v.end(),kom);
    int besii=count(v.begin(),v.end(),besi);


    if(kom==besi) cout<<(ll) n*(n-1)<<endl;
    else cout<< (ll) 2*komm*besii<<endl;
}
int main()
{
    AMGR();
